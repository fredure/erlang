...
handle_call({get_visited}, _From, [MaxNum, CurNum, Visited]) -> 
	{reply, Visited, [MaxNum, CurNum, Visited]};
handle_call({can_visit, URL}, _From, [MaxNum, CurNum, Visited]) -> 
	{reply, not(lists:member(URL, Visited)) and (CurNum < MaxNum), [MaxNum, CurNum, Visited]};
...
code_change(_OldCode, State, _Extra) -> 
	{ok, State}.