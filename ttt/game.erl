...
% установка значения в нужную позицию
...
% отправляет сообщение себе (и противнику, если соедиенение установлено)
tell(Message) ->
	{player, node()} ! {sendmsg, Message}.

% запускает новый узел и ждет противника
newgame()->
	register(player, spawn(game, wait_opponent, [])).  

% соединяется с другим узлом и создает игру с противником
playwith(XNode)->
	register(player, spawn(game, connect_opponent, [XNode])).
